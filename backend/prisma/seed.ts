...
async function main() {
  ...
  await prisma.interview.create({ data: { resumeId: r1.id, interviewerId: interviewer.id, round: 1, scheduledAt: new Date(Date.now() - 86400000 * 2), duration: 60, type: InterviewType.TECHNICAL, result: InterviewResult.PASS, score: 8, professionalScore: 8, communicationScore: 7, cultureScore: 9, feedback: '技术功底扎实，组件设计思路清晰，沟通表达顺畅，对团队文化有较高认同度。', notes: '重点考察组件架构与状态管理。' } });
  await prisma.interview.create({ data: { resumeId: r1.id, interviewerId: interviewer.id, round: 2, scheduledAt: new Date(Date.now() + 86400000), duration: 60, type: InterviewType.ONSITE, result: InterviewResult.PENDING, notes: '第二轮现场面试，考察系统设计与团队协作能力。' } });
